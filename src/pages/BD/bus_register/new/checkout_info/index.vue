  ...
  <el-col :span="24">
    ...
    <el-form id="IDForm" :model="IDForm" :rules="IDRules" ref="IDForm"
      ...
      <el-form-item label="身份信息" required>
        <el-radio-group v-model="IDForm.IDRadio">
...
<script>
  ...
  export default{
    ...
    data() {
      ...
      return {
        moduleV: {
          bank_flag: false    // 银行板块验证结果
        },
        checkFromFlag: false,  // 银行信息验证结果
        IDFromFlag: false,     // 身份信息验证结果
        checkForm: {
          bankRadio: "noBank",
          account_type: "",
          bank: [],
      ...
      filling: function() {
        ...
        if (userinfo.card_code) {         // 有身份信息
          ...
          if (userinfo.cert_type === "PASSPORT") {
            self.IDForm.card_all_url = userinfo.card_front_url  // 证件图片
            self.IDForm.cardImg = false
          } else {
            self.IDForm.cardImg = true
            self.IDForm.card_front_url = userinfo.card_front_url  // 证件图片
    ...
    methods: {
      ...
      checkFormV: function() {
        ...
        self.$refs.checkForm.validate((valid) => {
          if (valid && self.moduleV.bank_flag) {
            self.checkFromFlag = true
          }
        })
      },
      IDFormV: function() {
        ...
        self.$refs.IDForm.validate((valid) => {
          if (valid && imageFlag) {
            self.IDFromFlag = true
          }
        })
      },
      checkValidate: function(flag) {
        var self = this
        if (self.checkForm.bankRadio === "hasBank") {
          self.checkFormV()
        } else {
          self.checkFromFlag = true
        }
        if (self.IDForm.IDRadio === "hasID") {
          self.IDFormV()
        } else {
          self.IDFromFlag = true
        }
        if (self.checkFromFlag && self.IDFromFlag) {
          alert(1)
        }
        //              var formData = {
//                step: "BANK",
...
//                "type": flag,
//                "bankinfo": {
//                  "account_type": self.checkForm.account_type,
//                  "admiprovince_id": self.checkForm.bank[0],
//                  "admicity_id": self.checkForm.bank[1],
//                  "bank_id": self.checkForm.bank[2],
//                  "branch_id": self.checkForm.bank[3],
//                  "custom_branch": self.checkForm.bank[4],
//                  "person_or_company_name": self.checkForm.person_or_company_name,
//                  "bank_account": self.checkForm.bank_account,
//                  "billing_account_name": self.checkForm.billing_account_name,
//                  "billing_account_tel": self.checkForm.billing_account_tel
//                },
//                "userinfo": {
//                  "real_name": self.IDForm.real_name,
//                  "cert_type": self.IDForm.cert_type,
//                  "card_code": self.IDForm.card_code,
//                  "card_front_url": self.IDForm.card_front_url,
//                  "card_back_url": self.IDForm.card_back_url,
//                }
//              }
